#include "oplus_cam_insensor_eeprom.h"
...
int oplus_cam_eeprom_sc820cs(struct cam_eeprom_ctrl_t *e_ctrl, uint8_t *data)
{
	...
	CAM_ERR(CAM_EEPROM, "start oplus_cam_eeprom_sc820cs, i2c:0x%x", e_ctrl->io_master_info.qup_client->i2c_client->addr);
		...
		for (int idx=0; idx < page_count; idx++) {
			...
			if(!pre_check_flag){
				...
				if (rc) {
					...
					pre_check_flag = 1;
					CAM_ERR(CAM_EEPROM, "group[%d] flag check success", select_group);
				}
			}
			...
			if((idx+1)%page_num_pergroup == 0){
				...
				if (rc) {
					...
					CAM_ERR(CAM_EEPROM, "group[%d] read and checksum success", select_group);
					break ;
				}
			}
		}
